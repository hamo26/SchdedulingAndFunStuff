...
network_buffer base_network_buffer;
base_network_buffer.region_mutex = PTHREAD_MUTEX_INITIALIZER;
base_network_buffer.space_available = PTHREAD_COND_INITIALIZER;
base_network_buffer.data_available = PTHREAD_COND_INITIALIZER;
...
void add_packet_to_buffer(packet_t* packet){
  packet_copy(packet,&(network_buffer.network_buffer[rear]));
  rear = (rear+1) % SIZE;
  size++;
}
...
packet_t get_packet_from_buffer(){
  packet_t packet;
  packet_copy(&(network_buffer.network_buffer[front]),&packet);
  front= (front+1) % SIZE;
  size--;
  return packet;
}
	...
	while(1){
		...
		for (i = 0; i < 4; i++) {
			...
			if (in_port[i].flag == TRUE) {
				...
				pthread_mutex_lock(&base_network_buffer.region_mutex);
				if (size == SIZE) {	
					printf("The buffer is full\n");
				       	printf("Thread in locked\n");
					pthread_cond_wait(&(base_network_buffer.space_available),&(base_network_buffer.region_mutex));
					printf("Thread in unlocked\n");
				}
				add_packet_to_buffer(&(in_port[i].packet));
				pthread_cond_signal(&(base_network_buffer.data_available));
				pthread_mutex_unlock(&(base_network_buffer.region_mutex));
...
//	   printf("Successfuly retrieved lock for regional mutex in out thread\n");
           if (size == 0) {
           	printf("Thread out locked.\n");
                pthread_cond_wait(&(base_network_buffer.data_available),&(base_network_buffer.region_mutex));
                printf("Thread out unlocked.\n");
           }

           packet = get_packet_from_buffer();
           pthread_cond_signal(&(base_network_buffer.space_available));
           pthread_mutex_unlock(&(base_network_buffer.region_mutex));
